...
import { useWorldBossCompletions } from '@renderer/hooks/useWorldBossCompletions'
import './DailyWorldBossCard.css'
...
const DailyWorldBossCard: React.FC<DailyWorldBossCardProps> = ({ title }) => {
  ...
  const completedCount = bosses.filter(b => b.completed).length
    ...
    <Card title={title}>
      ...
      <div className="daily-card-content">
        <div className="daily-progress">
          ...
          <p className="progress-text">{completedCount} / {bosses.length} bosses</p>
        </div>

        <div className="daily-bosses-list">
          {bosses.map(boss => (
            <div key={boss.id} className={`boss-item ${boss.completed ? 'completed' : ''}`}>
              <div className="boss-checkbox">
                <span className={`checkbox-icon ${boss.completed ? 'checked' : ''}`}>
                  {boss.completed ? '✓' : ''}
                </span>
              </div>
...
export default DailyWorldBossCard
